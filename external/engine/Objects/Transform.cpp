...
namespace Engine
{
	...
	glm::mat4 Transform::LocalTransformGLM() const
	{
		glm::vec3 pos = glm::vec3(GetPosition().X, GetPosition().Y, GetPosition().Z);
		glm::vec3 scale = glm::vec3(GetScale().X, GetScale().Y, GetScale().Z);
		Quaternion q = GetOrientation();

		// Convert Quaternion to GLM format
		glm::quat glmQuat(q.W, q.X, q.Y, q.Z);

		glm::mat4 translation = glm::translate(glm::mat4(1.0f), pos);
		glm::mat4 rotation = glm::toMat4(glmQuat);
